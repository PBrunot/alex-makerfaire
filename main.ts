...
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if (!(story.isMenuOpen()) && introComplete) {
        if (direzioneColpo == 1) {
            let spada: Image = null
            projectile = sprites.createProjectileFromSprite(spada, eroe, 100, 0)
            pause(100)
...
sprites.onDestroyed(SpriteKind.GuardiaPortale, function (sprite37) {
    ...
    if (zonaCorrente == 7 && morteGuardie == 9) {
        ...
        tiles.setTileAt(tiles.getTileLocation(19, 22), assets.tile`myTile23`)
        tiles.setWallAt(tiles.getTileLocation(19, 22), false)
    }
})
// Villaggio
